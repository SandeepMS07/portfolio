...
export function BackgroundFX() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      <motion.div
        className="absolute left-1/4 top-10 h-64 w-64 rounded-full bg-linear-to-br from-cyan-400/30 via-transparent to-indigo-400/20 blur-3xl"
        animate={{ rotate: [0, 12, -8, 0], scale: [1, 1.05, 0.98, 1] }}
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute right-1/4 bottom-12 h-72 w-72 rounded-full bg-linear-to-tl from-indigo-500/25 via-transparent to-cyan-300/20 blur-3xl"
        animate={{ rotate: [0, -10, 6, 0], scale: [1, 0.97, 1.04, 1] }}
        transition={{ duration: 24, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute inset-x-0 top-1/3 h-20 bg-linear-to-r from-transparent via-cyan-400/15 to-transparent blur-2xl"
        animate={{
          x: ["-10%", "10%", "-8%", "0%"],
          opacity: [0.35, 0.6, 0.4, 0.5],
        }}
